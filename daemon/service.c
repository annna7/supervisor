...
service_t service_create(const char * service_name, const char * program_path, const char ** argv, int argc, int flags, time_t start_time) {
    ...
    if ((flags & SUPERVISOR_FLAGS_CREATESTOPPED) == 1) {
        ...
        append_to_global_response_str("Service %s is pending for %d seconds\n", service_name,(flags & SUPERVISOR_FLAGS_CREATESTOPPED)-1);
    ...
    if (pid == 0) {
        ...
        if ((flags & SUPERVISOR_FLAGS_CREATESTOPPED) == 1) {
            ...
            syslog(LOG_INFO, "Service %s is starting now", service_name);
            snprintf(status_message, sizeof(status_message), "RUNNING %d", getpid());
            write(scheduling_pipe_fd[1], status_message, strlen(status_message));
        }
